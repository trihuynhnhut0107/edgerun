 ...
 */
function calculateTimeWindowConfidence(calculated: CalculatedTimeWindow): number {
...
export async function calculateBestInsertion(
  ...
  for (let i = 0; i <= currentRoute.length; i++) {
    for (let j = i + 1; j <= currentRoute.length + 1; j++) {
      ...
      const totalDistance = await calculateRouteTotalDistance(candidateLocations);
      const originalDistance = await calculateRouteTotalDistance(currentLocations);
...
export async function draftBestAssignments(
  ...
  for (const region of regions) {
    ...
    for (const order of regionOrders) {
      ...
      console.log(`    📝 Calculating drafts for order ${order.id} with ${eligibleDrivers.length} drivers...`);
  ...
  if (unassignedOrders.length > 0) {
    ...
    for (const order of unassignedOrders) {
      console.warn(`    - Order ${order.id}: No eligible drivers in any region`);
    }
  }
...
export async function runMatchingCycle(): Promise<void> {
  ...
  while (offerRound <= MAX_ROUNDS) {
    ...
    // PHASE 3: Wait (skip for manual testing - will be triggered separately)
    // await waitForResponses(3 * 60 * 1000);

    // PHASE 4: Process (skip for manual testing - will be triggered separately)
    // const { rejected, expired } = await processResponses();

    // if (rejected + expired === 0) {
    //   console.log(`\n✅ No rejections. Stopping.`);
    //   break;
    // }

    offerRound++;

    // For manual testing, break after first draft-offer cycle
    console.log(
      `\n⏸️  Pausing for manual testing. Call processResponses() to continue.`
    );
    break;
  }
 ...
 */
export async function matchOrders(autoAccept: boolean = true): Promise<OptimizedRoute[]> {
  console.log(
    "🚀 Starting REGION-BASED matching engine (Draft→Offer→Accept flow)..."
  );

  // PHASE 1: Draft best assignments using regions
  console.log("\n📍 PHASE 1: DRAFT (Region-based with PostGIS)");
  const drafts = await draftBestAssignments(1);
  ...
  // PHASE 2: Offer assignments (OFFERED status)
  console.log("\n📍 PHASE 2: OFFER (Create OFFERED records)");
  const offeredCount = await offerAssignments(drafts, 1);
  ...
  // Group assignments by driver to show routes
  const routes = await buildRoutesFromAssignments(autoAccept);

  // Print summary
  console.log("\n📊 MATCHING SUMMARY (Region-based):");
  let totalDistance = 0;
  ...
  console.log(`   ASSIGNMENT STATUS: ${autoAccept ? AssignmentStatus.ACCEPTED : AssignmentStatus.OFFERED}`);
  console.log(`   ORDER STATUS: ${autoAccept ? OrderStatus.ASSIGNED : OrderStatus.OFFERED}`);
 ...
 */
async function buildRoutesFromAssignments(autoAccept: boolean): Promise<OptimizedRoute[]> {
  const assignmentRepo = AppDataSource.getRepository(OrderAssignment);
  ...
  for (const route of routes) {
    if (route.sequence.length > 1) {
      ...
      route.metrics.distancePerOrder = route.totalDistance / route.metrics.orderCount;
    }
  }
...
async function autoAcceptAllOffers(): Promise<number> {
  ...
  for (const assignment of offeredAssignments) {
    ...
    if (assignment.order) {
      ...
      await orderRepo.save(assignment.order);
    }
