import { Controller, Post, Get, Patch, Route, Body, Path, Response, Tags } from 'tsoa';
...
/**
 * Driver route response with all assigned orders
 */
...
export class DriverController extends Controller {
  ...
  async createDriver(@Body() body: CreateDriverRequest): Promise<DriverResponse> {
  ...
  async getOfferedAssignments(@Path() id: string): Promise<RouteAssignment[]> {
    ...
    return assignments.map((assignment) => {
      ...
      const dropoffCoords = assignment.order?.dropoffLocation?.coordinates || [0, 0];
   ...
   * Returns all active (non-delivered) orders assigned to the driver in sequence order
   *
   * @param id Driver ID
   * @returns DriverRouteResponse with all assignments and order details
   */
  @Get('{id}/route')
  ...
  async getDriverRoute(@Path() id: string): Promise<DriverRouteResponse> {
    ...
    const assignments = await orderAssignmentService.getActiveAssignmentsForDriver(id);

    // Transform assignments to include order details
    const routeAssignments: RouteAssignment[] = assignments.map((assignment) => {
      const pickupCoords = assignment.order?.pickupLocation?.coordinates || [0, 0];
      const dropoffCoords = assignment.order?.dropoffLocation?.coordinates || [0, 0];
